...
def launch_toeic_full_test(pack_id: str, payload: FullTestLaunchRequest) -> dict[str, Any]:
    ...
    if mode == "practice":
        ...
        for part_label in payload.selected_parts:
            if part_label in available_parts and part_label not in normalized_selected_parts:
                normalized_selected_parts.append(part_label)
        selected_parts = normalized_selected_parts or available_parts
    else:
    ...
    if mode == "exam":
        ...
        ratio = (selected_question_count / max(int(pack.get("questions") or 1), 1)) if pack.get("questions") else 1
        xp = max(8, round(14 * ratio))
    _record_progress_event(
        DEFAULT_PROGRESS_USER_ID,
        "quiz",
        f"{'Luyen thi' if mode == 'exam' else 'Luyen tap'} • {pack['title']} • {selected_question_count} cau",
        xp,
    )
