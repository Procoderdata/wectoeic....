...
class DbToeicFullTestResume(Base):
    __tablename__ = "toeic_full_test_resumes"
    __table_args__ = (
        UniqueConstraint("session_id", name="uq_toeic_full_test_resume_session"),
    )
    ...
    mode: Mapped[str] = mapped_column(String(24), nullable=False, default="practice")
    selected_parts: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
    selected_questions: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    duration_minutes: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    current_group_index: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    answers: Mapped[dict[str, str]] = mapped_column(JSON, nullable=False, default=dict)
    notes: Mapped[dict[str, str]] = mapped_column(JSON, nullable=False, default=dict)
...
def _full_test_resume_row_to_dict(row: Optional[DbToeicFullTestResume]) -> Optional[dict[str, Any]]:
    if row is None:
        return None
    return {
        ...
        "series": row.series,
        "mode": row.mode,
        "selected_parts": row.selected_parts if isinstance(row.selected_parts, list) else [],
        "selected_questions": _safe_int(row.selected_questions),
        "duration_minutes": _safe_int(row.duration_minutes),
        "current_group_index": _safe_int(row.current_group_index),
...
def save_full_test_resume(payload: dict[str, Any]) -> Optional[dict[str, Any]]:
    if not is_enabled():
        return None
    ...
    selected_parts = payload.get("selected_parts") if isinstance(payload.get("selected_parts"), list) else []
    answers = payload.get("answers") if isinstance(payload.get("answers"), dict) else {}
    notes = payload.get("notes") if isinstance(payload.get("notes"), dict) else {}

    with SessionLocal() as db:
        row = db.execute(
            select(DbToeicFullTestResume).where(DbToeicFullTestResume.session_id == session_id)
        ).scalar_one_or_none()
        if row is None:
            row = DbToeicFullTestResume(
                session_id=session_id,
                user_id=user_id,
                pack_id=pack_id,
                title=title or "TOEIC Full Test",
                series=series or "ETS",
                mode=str(payload.get("mode") or "practice"),
                selected_parts=selected_parts,
                selected_questions=_safe_int(payload.get("selected_questions")),
                duration_minutes=_safe_int(payload.get("duration_minutes")),
                current_group_index=max(0, _safe_int(payload.get("current_group_index"))),
                answers=answers,
                notes=notes,
            ...
            row.series = series or row.series
            row.mode = str(payload.get("mode") or row.mode or "practice")
            row.selected_parts = selected_parts
            row.selected_questions = _safe_int(payload.get("selected_questions"))
            row.duration_minutes = _safe_int(payload.get("duration_minutes"))
            row.current_group_index = max(0, _safe_int(payload.get("current_group_index")))
            row.answers = answers
            row.notes = notes
        ...
        db.commit()
        db.refresh(row)
        return _full_test_resume_row_to_dict(row)


def get_full_test_resume(session_id: str, user_id: Optional[str] = None) -> Optional[dict[str, Any]]:
    if not is_enabled():
        return None
    ...
    with SessionLocal() as db:
        row = db.execute(
            select(DbToeicFullTestResume).where(DbToeicFullTestResume.session_id == normalized_session_id)
        ).scalar_one_or_none()
        if row is None:
            return None
...
def get_latest_full_test_resume(user_id: str) -> Optional[dict[str, Any]]:
    if not is_enabled():
        return None
    ...
    with SessionLocal() as db:
        row = db.execute(
            select(DbToeicFullTestResume)
            .where(DbToeicFullTestResume.user_id == normalized_user_id)
            .order_by(DbToeicFullTestResume.updated_at.desc())
            .limit(1)
        ).scalar_one_or_none()
        return _full_test_resume_row_to_dict(row)


def delete_full_test_resume(session_id: str) -> None:
    if not is_enabled():
        return
