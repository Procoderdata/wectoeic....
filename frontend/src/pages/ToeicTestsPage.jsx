...
export default function ToeicTestsPage() {
  ...
  useEffect(() => {
    ...
    toeicAPI.getFullTestResume('demo-user')
      .then((data) => {
        setResumeItem(data?.item || null);
      })
      .catch(() => {
        setResumeItem(null);
      })
  ...
  const launchTest = async (mode, packId, options = {}) => {
    ...
    try {
      const launchResult = await toeicAPI.launchFullTest(packId, mode, 'demo-user', options.selectedParts || []);
  ...
  return (
    ...
    >
      <section className="soft-card toeic-tests-board">
        ...
        {!loading && !resumeLoading && resumeItem ? (
          <article className="full-test-resume-card">
            <div>
              <p className="section-kicker">Bài đang làm dở</p>
              <h4>{resumeItem.title}</h4>
              <p className="subtle">
                {resumeItem.progress_label} • {resumeItem.answered_count}/{resumeItem.selected_questions || 0} câu đã chọn đáp án
              </p>
            </div>
            <button
              className="primary-btn wide-btn"
              onClick={() => navigate(`/toeic/tests/session/${resumeItem.session_id}`)}
            >
              Tiếp tục
            </button>
          </article>
          ...
          <article className="overlay-panel toeic-part-picker-panel" onClick={(event) => event.stopPropagation()}>
            <div className="toeic-part-picker-head">
              ...
              <div className="toeic-part-picker-actions">
                ...
                >
                  {launchingId === practicePicker.test?.id ? 'Đang tạo phiên...' : 'Bắt đầu'}
                </button>
              </div>
            </div>
