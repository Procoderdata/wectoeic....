...
export const toeicAPI = {
  ...
  async getFullTestResume(userId = 'demo-user') {
    const params = new URLSearchParams();
    if (userId) params.set('user_id', userId);
    const queryString = params.toString();
    return fetchJson(`/api/toeic/full-tests/resume${queryString ? `?${queryString}` : ''}`);
  },
  ...
  async getFullTestSession(sessionId) {
    return fetchJson(`/api/toeic/full-tests/sessions/${sessionId}`);
  },

  async submitFullTestSession(sessionId, answers = []) {
    return fetchJson(`/api/toeic/full-tests/sessions/${sessionId}/submit`, {
      method: 'POST',
      body: JSON.stringify({ answers }),
    });
  },

  async saveFullTestSession(sessionId, payload = {}) {
    return fetchJson(`/api/toeic/full-tests/sessions/${sessionId}/save`, {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  },
